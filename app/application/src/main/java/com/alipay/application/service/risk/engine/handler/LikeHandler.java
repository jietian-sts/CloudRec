 ...
 * Checks if the fact value contains the condition value
 */
public class LikeHandler extends AbstractHanlder {

    /**
     * Check if this handler can handle the given condition
     * @param conditionItem the condition item to check
     * @return true if this handler can handle LIKE operator
     */
    @Override
    protected boolean canHandle(ConditionItem conditionItem) {
        return conditionItem.getOperator() == Operator.LIKE;
    }
     ...
     * @return true if the fact value contains the condition value
     */
    @Override
