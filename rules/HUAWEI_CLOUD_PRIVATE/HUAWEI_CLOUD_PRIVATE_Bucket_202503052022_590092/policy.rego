...
risk if {
	bucket_allows_public_write_access
}
...
bucket_allows_public_write_access if {
	# Case 1: public write access allowed via ACL
	count(risk_msgs) > 0
}

bucket_allows_public_write_access if {
	# Case 2: public write access allowed via access policy
	count(statement_allow_write) > 0
}
...
obs_write_actions := {"*", "s3:*", "s3*", "s3:delete", "s3:put"}

obs_write_action(statement) := actions if {
	actions := [action |
		some action in statement.Action
		startswith(lower(action), obs_write_actions[_])
	]
}

obs_write_action(statement) := statement.Action if {
	startswith(lower(statement.Action), obs_write_actions[_])
}
...
msg_to_user contains info if {
	some stmt in statement_allow_write
