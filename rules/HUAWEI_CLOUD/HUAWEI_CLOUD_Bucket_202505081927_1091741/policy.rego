...
risk if {
	count(statement_allow_get_action_for_all) > 0
}

statement_allow_get_action_for_all contains risk_action if {
	bucket_policy := json.unmarshal(input.BucketPolicy)

	some statement in bucket_policy.Statement
	risk_action := obs_get_action(statement)
	count(risk_action) > 0
...
obs_get_actions := {"*", "s3:*", "s3*", "s3:get"}

obs_get_action(statement) := actions if {
	actions := [action |
		some action in statement.Action
		startswith(lower(action), obs_get_actions[_])
	]
}

obs_get_action(statement) := statement.Action if {
	startswith(lower(statement.Action), obs_get_actions[_])
}
...
msg_to_user contains info if {
	some stmt in statement_allow_get_action_for_all
