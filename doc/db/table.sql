...
CREATE
database if NOT EXISTS `cloudrec_db` default character set utf8mb4 collate utf8mb4_unicode_ci;
use
`cloudrec_db`;
--
-- 修改 root 用户权限（允许从任意 IP 访问）
...
/********************************************************************/
/*   DatabaseName = cloudrec_db   */
/*   TableName = cloud_resource_instance   */
/********************************************************************/
CREATE TABLE IF NOT EXISTS `cloud_resource_instance` (
                                                         `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
    `gmt_create` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `gmt_modified` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    `platform` varchar(255) DEFAULT NULL COMMENT '平台',
    ...
    `resouce_id` varchar(512) DEFAULT NULL COMMENT '资产id',
    `resouce_name` varchar(512) DEFAULT NULL COMMENT '资产名称',
